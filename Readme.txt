...
The minimax_abp function implements the minimax algorithm with alpha-beta pruning. The play_game function starts a new game and alternates between the human and AI players until the game ends. The get_possible_moves function returns the possible moves for a given state, and the evaluate function assigns a score to each state based on the number of balls left for each colour.
...
* get_possible_moves: This function takes a game state and returns a list of all possible moves (i.e., which colour ball to pick) for the current player.
* is_game_over: This function takes a game state and returns True if the game is over (i.e., all balls of one colour have been picked).
